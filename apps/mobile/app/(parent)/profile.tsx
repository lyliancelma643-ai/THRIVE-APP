...
export default function ParentProfile() {
  ...
  async function handleSave() {
    ...
    const { error } = await updateProfile({
      full_name: fullName,
      phone,
      notification_prefs: { push: pushEnabled, email: emailEnabled },
    });
    if (error) {
    ...
    >
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.avatarSection}>
          ...
          <Text style={styles.avatarName}>{profile?.full_name || 'Parent'}</Text>
            ...
            <React.Fragment key={i}>
              <View style={styles.toggleRow}>
                ...
                <Switch
                  ...
                  thumbColor={item.value ? '#10b981' : '#6b7280'}
                  disabled={!editMode}
                />
              </View>
              {i < 1 && <View style={styles.fieldDivider} />}
            </React.Fragment>
          ))}
        </View>
        ...
        <View style={styles.footer}>
          <Text style={styles.footerText}>Thrive v1.0.0</Text>
        </View>
      </ScrollView>
    </KeyboardAvoidingView>
  );
...
const styles = StyleSheet.create({
  ...
  editBtn: { paddingHorizontal: 16, paddingVertical: 8, borderRadius: 20, backgroundColor: '#1e293b', borderWidth: 1, borderColor: '#334155' },
  editBtnActive: { backgroundColor: 'rgba(16,185,129,0.15)', borderColor: '#10b981' },
  editBtnText: { color: '#94a3b8', fontSize: 14, fontWeight: '600' },
  editBtnTextActive: { color: '#6ee7b7' },
  avatarSection: { alignItems: 'center', paddingVertical: 24 },
  avatar: { width: 88, height: 88, borderRadius: 44, backgroundColor: '#10b981', alignItems: 'center', justifyContent: 'center', marginBottom: 12 },
  avatarText: { color: '#fff', fontSize: 30, fontWeight: '700' },
  avatarName: { color: '#f8fafc', fontSize: 20, fontWeight: '700', marginBottom: 8 },
  roleBadge: { paddingHorizontal: 14, paddingVertical: 5, borderRadius: 20, backgroundColor: 'rgba(16,185,129,0.15)', borderWidth: 1, borderColor: 'rgba(16,185,129,0.3)' },
  roleBadgeText: { color: '#6ee7b7', fontSize: 13, fontWeight: '600' },
  section: { backgroundColor: '#1e293b', borderRadius: 16, marginHorizontal: 16, marginBottom: 16, overflow: 'hidden' },
  sectionTitle: { color: '#475569', fontSize: 11, fontWeight: '700', letterSpacing: 1, paddingHorizontal: 16, paddingTop: 16, paddingBottom: 10 },
  field: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 14 },
  fieldLabel: { color: '#94a3b8', fontSize: 15 },
  fieldValue: { color: '#f8fafc', fontSize: 15, fontWeight: '500' },
  fieldValueMuted: { color: '#64748b' },
  fieldInput: { color: '#f8fafc', fontSize: 15, textAlign: 'right', flex: 1, marginLeft: 16, borderBottomWidth: 1, borderBottomColor: '#10b981', paddingBottom: 4 },
  fieldDivider: { height: 1, backgroundColor: '#334155', marginHorizontal: 16 },
  toggleRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 12 },
