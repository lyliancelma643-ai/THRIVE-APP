...
export default function ParentReportsPage() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-2xl font-bold text-[#F7F5F2]">Rapports</h1>
        <p className="text-[#a7c4bc] mt-1">Bilans et rapports de progression.</p>
      </div>

      <div className="space-y-4">
        {reports.map((report, i) => (
          <div key={i} className="bg-white/10 backdrop-blur-md rounded-xl border border-[#a7c4bc]/10 shadow-sm p-6 flex items-center gap-4">
            <div className="w-10 h-10 rounded-lg bg-purple-50 flex items-center justify-center text-xl flex-shrink-0">
              📄
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                <h3 className="font-semibold text-[#F7F5F2] text-sm">{report.title}</h3>
                {report.new && (
                  <span className="text-xs bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full font-medium">Nouveau</span>
                )}
              </div>
              <p className="text-xs text-[#a7c4bc] mt-0.5">{report.child} · {report.type} · {report.date}</p>
            </div>
