...
// plates #0C2029 sur fond #06161E, un seul accent (#F9EB50), le sage (#A7C4BC)
...
.bilan-root{font-family:var(--font-inter),'Inter',system-ui,sans-serif;color:#EAF3F1;--bcard-pad:20px;--bnode:36px;max-width:100%;}
...
.bilan-root .b-card{background:#0C2029;border-radius:22px;padding:var(--bcard-pad,20px);position:relative;}
/* Étiquette de section — seul niveau de titre secondaire */
.bilan-root .b-eye{font-size:12px;font-weight:700;letter-spacing:.12em;text-transform:uppercase;color:rgba(234,243,241,.55);margin:0;}
.bilan-root .b-track{height:6px;border-radius:3px;background:rgba(255,255,255,.09);overflow:hidden;}
...
.bilan-root .b-card.b-clk:hover{background:#0F2833;}
...
.bilan-root .b-hint{position:absolute;right:16px;top:18px;z-index:3;color:rgba(234,243,241,.35);line-height:0;pointer-events:none;transition:color .18s ease;}
.bilan-root .b-clk:hover .b-hint{color:rgba(234,243,241,.7);}
...
.b-modal{box-sizing:border-box;font-family:var(--font-inter),'Inter',system-ui,sans-serif;color:#EAF3F1;position:relative;width:100%;max-width:560px;max-height:86vh;overflow-y:auto;overscroll-behavior:contain;border-radius:26px;background:#0C2029;box-shadow:0 40px 90px rgba(0,0,0,.55);padding:26px;animation:b-cardIn .3s cubic-bezier(.22,.61,.36,1) both;}
...
function permaGraphHtml(points: { session_number: number; value: number }[]): string {
  ...
  if (pts.length === 0) {
    return `<div style="height:150px;display:flex;align-items:center;justify-content:center;text-align:center;">
      <p style="margin:0;max-width:300px;font-size:14px;line-height:1.5;color:rgba(234,243,241,.55);">En attente du premier questionnaire de bien-être. Le coach en envoie un court après chaque séance.</p>
    </div>`;
  }
  ...
  const label = `<span class="bx" style="position:absolute;left:${((last.x / 660) * 100).toFixed(1)}%;top:${((last.y / 190) * 100).toFixed(1)}%;transform:translate(${nearRight ? '-88%' : '-50%'},${near ? '70%' : '-160%'});font-size:12px;font-weight:600;color:#F9EB50;white-space:nowrap;pointer-events:none;">S${last.s} · ${last.v}</span>`;

  return `<div style="position:relative;">
    <svg viewBox="0 0 660 190" width="100%" height="150" preserveAspectRatio="none" style="display:block;">
      <line x1="0" y1="40" x2="660" y2="40" stroke="rgba(255,255,255,.06)"></line>
      <line x1="0" y1="104" x2="660" y2="104" stroke="rgba(255,255,255,.06)"></line>
      <line x1="0" y1="150" x2="660" y2="150" stroke="rgba(255,255,255,.06)"></line>
      ${line}
      ${dot}
    </svg>
    ${label}
...
function permaPillarsHtml(pillars: Record<string, number>): string {
  ...
  const rows = ORDER.map(([k, label]) => {
    ...
    return `<div style="display:flex;align-items:center;gap:12px;">
      <span style="width:132px;flex-shrink:0;font-size:13px;font-weight:500;color:rgba(234,243,241,.7);">${label}</span>
      ...
      <span class="disp" style="width:26px;text-align:right;font-size:13px;font-weight:600;color:${has ? '#F7F5F2' : 'rgba(234,243,241,.3)'};">${has ? v : '—'}</span>
    </div>`;
  }).join('');
...
export function buildHtml(d: {
  ...
  const lockNote = (txt: string) =>
    `<div style="display:flex;align-items:flex-start;gap:10px;padding:12px 14px;border-radius:14px;background:rgba(249,235,80,.07);border:1px solid rgba(249,235,80,.28);">
      <svg viewBox="0 0 24 24" fill="none" style="width:16px;height:16px;flex-shrink:0;margin-top:2px;color:#F9EB50;" aria-hidden="true"><rect x="4.5" y="10.5" width="15" height="9.5" rx="2.4" stroke="currentColor" stroke-width="1.8"></rect><path d="M8 10.5V7.5a4 4 0 0 1 8 0v3" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"></path></svg>
      <span style="font-size:14px;line-height:1.5;color:rgba(234,243,241,.88);"><b style="color:#F9EB50;">Contenu réservé.</b> ${txt} <span class="b-hover" data-href="/parent/upgrade" style="color:#F9EB50;font-weight:600;text-decoration:underline;text-underline-offset:2px;">Voir les forfaits</span></span>
    </div>`;
  ...
  const skillRow = (label: string, v: number) =>
    `<div style="display:flex;align-items:center;gap:12px;">
      <span style="flex:1;min-width:0;font-size:14px;font-weight:500;color:rgba(234,243,241,.8);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${esc(label)}</span>
      ...
      <span class="disp" style="width:26px;text-align:right;font-size:14px;font-weight:600;color:#F7F5F2;flex-shrink:0;">${Math.round(v)}</span>
    </div>`;
  ...
  for (let n = 1; n <= 13; n++) {
    ...
    if (done) {
      ...
      circle = `<span class="disp bx" style="${node}border:1px solid rgba(255,255,255,.16);color:rgba(234,243,241,.55);font-weight:600;">${n}</span>`;
    }
  ...
  const okBadge = (txt: string) =>
    `<span style="flex-shrink:0;font-size:13px;font-weight:600;color:#F9EB50;white-space:nowrap;">${txt}</span>`;
  const waitBadge = (txt: string) =>
    `<span style="flex-shrink:0;font-size:13px;font-weight:600;color:rgba(234,243,241,.6);white-space:nowrap;">${txt}</span>`;
  ...
  const toolHead = (title: string, badge: string) =>
    `<div style="display:flex;flex-wrap:wrap;align-items:baseline;justify-content:space-between;gap:4px 10px;margin-bottom:16px;padding-right:26px;">
      <span class="disp" style="font-size:19px;font-weight:600;color:#F7F5F2;">${title}</span>${badge}
    </div>`;
  ...
  const toolboxItemsHtml = toolbox.length
    ? `<div style="display:flex;flex-direction:column;gap:10px;">${toolbox
        .map(
          (t) =>
            `<div style="display:flex;gap:10px;align-items:baseline;"><span style="width:5px;height:5px;border-radius:50%;background:#A7C4BC;margin-top:7px;flex-shrink:0;"></span><span style="font-size:15px;line-height:1.45;color:rgba(234,243,241,.86);">${esc(t.tool)}${t.context ? `<span style="color:rgba(234,243,241,.6);"> — ${esc(t.context)}</span>` : ''}</span></div>`
        )
        .join('')}</div>`
    : '';
  const lifeSkillHtml = lifeSkillGoal
    ? `<p class="b-eye" style="margin-bottom:6px;">Objectif life skill</p><p style="margin:0 0 18px;font-size:15px;line-height:1.5;color:rgba(234,243,241,.86);">${esc(lifeSkillGoal)}</p>`
    : '';
  const actionsHtml = myActions.length
    ? `<p class="b-eye" style="margin-bottom:10px;">Ce qui dépend de moi</p><div style="display:flex;flex-direction:column;gap:8px;margin-bottom:18px;">${myActions
        .map(
          (a) =>
            `<div style="display:flex;gap:10px;align-items:baseline;"><span style="color:#F9EB50;flex-shrink:0;line-height:0;"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="m5 12 5 5L20 7"></path></svg></span><span style="font-size:15px;color:rgba(234,243,241,.86);">${esc(a)}</span></div>`
        )
        .join('')}</div>`
    : '';
    ...
    <div class="bx b-clk b-card" data-info="identite" style="grid-column:span 6;${ain(7)}">${HINT}
      ...
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:22px;align-items:start;">
        <div>
          ...
          <p style="margin:0 0 16px;font-size:15px;line-height:1.6;color:rgba(234,243,241,.86);white-space:pre-line;">${sportStory ? esc(sportStory) : '<span style="color:rgba(234,243,241,.5);">À renseigner avec le coach.</span>'}</p>
          ${seasonDream ? `<div style="padding:16px 18px;border-radius:16px;background:rgba(249,235,80,.06);"><p class="b-eye" style="margin-bottom:4px;color:rgba(249,235,80,.8);">Rêve de saison</p><p class="disp" style="margin:0;font-style:italic;font-size:17px;line-height:1.4;color:#F7F5F2;">${esc(seasonDream)}</p></div>` : ''}
        </div>
        <div>
          <p class="b-eye" style="margin-bottom:10px;">Forces (VIA)</p>
          ${strengthChips.length ? `<div style="display:flex;flex-wrap:wrap;gap:8px;">${strengthChips.map((s) => `<span style="display:inline-flex;align-items:center;gap:6px;padding:6px 11px;border-radius:11px;background:rgba(167,196,188,.12);font-size:13px;font-weight:500;color:#A7C4BC;"><span style="width:6px;height:6px;border-radius:50%;background:#A7C4BC;"></span>${esc(s)}</span>`).join('')}</div>` : '<span style="font-size:15px;color:rgba(234,243,241,.5);">Forces à identifier ensemble.</span>'}
        </div>
      </div>
    </div>`;
...
<div class="bilan-root bx" style="padding-bottom:8px;">
  ...
  <div class="b-clk" data-info="programme" style="display:block;padding:4px 2px 0;${ain(0)}">
    ...
    <div style="display:flex;align-items:baseline;gap:6px;margin-top:4px;">
      <span class="disp b-pct" style="font-size:104px;line-height:.86;font-weight:600;color:#F7F5F2;">${pct}</span><span class="disp b-pctsign" style="font-size:32px;font-weight:500;color:rgba(234,243,241,.55);">%</span>
    </div>
    <p style="margin:12px 0 0;font-size:15px;font-weight:500;color:rgba(234,243,241,.75);">${completed} séance${completed > 1 ? 's' : ''} sur 13 · ${completed < 13 ? 'en cours' : 'terminé'}</p>
  ...
  <div style="margin-top:26px;display:flex;align-items:center;gap:10px;${ain(1)}">
    <div class="b-hover b-clk bx" data-info="identite" role="button" style="flex:1;min-width:0;display:flex;align-items:center;gap:12px;padding:12px 14px;border-radius:18px;background:#0C2029;">
      ${avatar}
      <span style="flex:1;min-width:0;display:flex;flex-direction:column;gap:2px;">
        <span class="disp" style="font-size:17px;font-weight:600;color:#F7F5F2;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;flex-shrink:0;">${esc(fullName)}</span>
        <span style="font-size:13px;color:rgba(234,243,241,.65);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;flex-shrink:0;">${
          nickname ? `<span style="font-style:italic;color:${esc(accentColor)};">« ${esc(nickname)} »</span> · ` : ''
        }${subLine}</span>
      </span>
      <span style="flex-shrink:0;color:rgba(234,243,241,.45);line-height:0;"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="m9 6 6 6-6 6"></path></svg></span>
    </div>
    <span class="b-hover bx" data-action="edit-passport" role="button" aria-label="Personnaliser le passeport" style="width:48px;height:48px;flex-shrink:0;border-radius:16px;background:rgba(249,235,80,.1);display:grid;place-items:center;color:#F9EB50;line-height:0;"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M4 20h4L20 8l-4-4L4 16z"></path></svg></span>
  </div>
  ...
  <div class="b-stack" style="margin-top:16px;">
    ...
    <div class="bx b-clk b-card" data-info="competences" style="${ain(2)}">${HINT}
      ...
      ${
        ...
        gaugeGlobal == null
          ? `<p style="margin:12px 0 0;font-size:15px;font-weight:500;color:rgba(234,243,241,.7);">En attente de la première mesure LSSS.</p>`
          : `<div style="display:flex;align-items:baseline;gap:10px;margin-top:10px;">
        <span class="disp" style="font-size:52px;line-height:1;font-weight:600;color:#F7F5F2;">${gaugeGlobal}</span>
      ...
      ${skillBreakdownHtml}
      <p style="margin:18px 0 0;font-size:13px;color:rgba(234,243,241,.6);">Basé sur le questionnaire LSSS · mesuré aux séances 1, 7 et 13.</p>
    </div>
    ...
    <div class="bx b-clk b-card" data-info="perma" style="${ain(3)}">${HINT}
      <div style="display:flex;flex-wrap:wrap;align-items:baseline;justify-content:space-between;gap:6px 12px;padding-right:26px;">
        ...
        <span style="font-size:13px;font-weight:600;color:rgba(234,243,241,.62);white-space:nowrap;flex-shrink:0;">EPOCH · ${
          ent.lsssCurve
      ...
      ${
        ...
        <div style="display:flex;flex-direction:column;justify-content:space-between;padding:2px 0 22px;font-size:12px;color:rgba(234,243,241,.55);text-align:right;width:40px;flex-shrink:0;"><span>Élevé</span><span>Moyen</span><span>Bas</span></div>
        <div style="flex:1;min-width:0;">
          ${permaGraphHtml(permaPoints)}
          <div style="display:flex;justify-content:space-between;margin-top:8px;font-size:12px;color:rgba(234,243,241,.5);"><span>S1</span><span>S3</span><span>S5</span><span>S7</span><span>S9</span><span>S11</span><span>S13</span></div>
        </div>
      </div>
      ${permaPoints.length ? permaPillarsHtml(permaPoints[permaPoints.length - 1].pillars) : ''}
      <p style="margin:16px 0 0;font-size:13px;line-height:1.5;color:rgba(234,243,241,.6);">Bien-être mesuré après chaque séance — échelle EPOCH (engagement, persévérance, optimisme, connexion aux autres, bonheur).</p>`
          : `${lockedBars}
  ...
  <div class="b-row2" style="margin-top:12px;">
    ...
    <div class="bx b-clk b-card" data-info="parcours" style="${ain(4)}">${HINT}
      <div style="display:flex;flex-wrap:wrap;align-items:baseline;justify-content:space-between;gap:6px 12px;padding-right:26px;">
        ...
        <span class="disp" style="font-size:15px;font-weight:600;color:#F9EB50;">${completed}/13</span>
      </div>
      <div class="b-nodes" style="margin-top:18px;">${nodes}</div>
      <div style="display:flex;flex-wrap:wrap;gap:16px;margin-top:20px;padding-top:16px;border-top:1px solid rgba(255,255,255,.07);">
        <span style="display:inline-flex;align-items:center;gap:7px;font-size:13px;color:rgba(234,243,241,.7);"><span style="width:8px;height:8px;border-radius:50%;background:#A7C4BC;"></span>Terminée</span>
        <span style="display:inline-flex;align-items:center;gap:7px;font-size:13px;color:rgba(234,243,241,.7);"><span style="width:8px;height:8px;border-radius:50%;background:#F9EB50;"></span>En cours</span>
        <span style="display:inline-flex;align-items:center;gap:7px;font-size:13px;color:rgba(234,243,241,.7);"><span style="width:8px;height:8px;border-radius:50%;border:1px solid rgba(255,255,255,.3);"></span>À venir</span>
      </div>
      <div style="display:flex;flex-direction:column;gap:6px;margin-top:14px;">
        <p style="margin:0;font-size:15px;font-weight:500;color:rgba(234,243,241,.8);">Séance en cours · <b style="color:#F7F5F2;">S${cur < 1 ? 1 : cur}</b></p>
        <p style="margin:0;font-size:15px;font-weight:500;color:rgba(234,243,241,.7);">Prochaine · <b style="color:#F7F5F2;">S${next}</b> à programmer</p>
      </div>
    </div>
    ...
    <div class="bx b-clk b-card" data-info="boite" style="${ain(5)}">${HINT}
      ...
      <div style="display:flex;align-items:baseline;gap:8px;margin-top:10px;">
        <span class="disp" style="font-size:44px;line-height:1;font-weight:600;color:#F7F5F2;">${toolboxCount}</span>
        <span style="font-size:15px;font-weight:500;color:rgba(234,243,241,.6);">/ 6 outils collectés</span>
      </div>
      ...
      <div style="display:flex;align-items:center;gap:12px;margin-top:18px;padding:14px 16px;border-radius:16px;background:rgba(249,235,80,.07);">
        <span style="font-size:13px;font-weight:500;color:rgba(234,243,241,.65);">Focus word</span>
        <span class="disp" style="margin-left:auto;font-size:19px;font-weight:600;letter-spacing:.04em;color:#F9EB50;overflow-wrap:anywhere;">${esc(focus)}</span>
      </div>
    </div>
          ...
          nextSteps.length
            ? nextSteps
                .map((s) => {
                  ...
                  const bg = done ? 'rgba(167,196,188,.08)' : 'rgba(255,255,255,.035)';
                  const dueTxt = s.due_date
                    ? ` <span style="color:rgba(234,243,241,.55);">· ${new Date(s.due_date).toLocaleDateString('fr-CA', { day: 'numeric', month: 'short' })}</span>`
                    : '';
                  return `<div style="display:flex;align-items:center;gap:12px;padding:13px 14px;border-radius:16px;background:${bg};"><span style="width:30px;height:30px;border-radius:10px;background:rgba(255,255,255,.06);display:grid;place-items:center;color:${iconColor};flex-shrink:0;line-height:0;">${icon}</span><span style="font-size:15px;line-height:1.4;color:rgba(234,243,241,.85);">${esc(s.label)}${dueTxt}</span></div>`;
                })
                .join('')
            : `<div style="display:flex;align-items:center;gap:12px;padding:13px 14px;border-radius:16px;background:rgba(255,255,255,.035);"><span style="width:30px;height:30px;border-radius:10px;background:rgba(255,255,255,.06);display:grid;place-items:center;color:#F9EB50;flex-shrink:0;line-height:0;"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="9"></circle><path d="M12 7v5l3 2"></path></svg></span><span style="font-size:15px;line-height:1.4;color:rgba(234,243,241,.7);">Le coach précisera bientôt les prochaines étapes.</span></div>`
        }
      </div>
      <div class="b-hover bx" data-href="/parent/fitness" style="display:flex;align-items:center;justify-content:center;gap:8px;min-height:48px;margin-top:16px;padding-top:12px;border-top:1px solid rgba(255,255,255,.07);font-size:15px;font-weight:600;color:#A7C4BC;">Voir le parcours complet <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M4 12h15M13 6l6 6-6 6"></path></svg></div>
    </div>
  </div>

  <!-- ROW 3 · MES OUTILS THRIVE -->
  <div style="margin:38px 2px 18px;${ain(7)}">
    <h2 class="disp b-toolstitle" style="margin:0 0 4px;font-size:26px;font-weight:600;color:#F7F5F2;">Mes outils THRIVE</h2>
    <p style="margin:0 0 6px;font-size:15px;line-height:1.5;color:rgba(234,243,241,.68);">Les livrables construits séance après séance — chacun son atelier.</p>
    <p style="margin:0;font-size:13px;font-weight:600;color:#F9EB50;">8 ateliers · ${renseignes} renseignés</p>
  </div>
  ...
  <div class="b-tools">
    ...
    <div class="bx b-clk b-card" data-info="objectif" style="grid-column:span 4;${ain(8)}">${HINT}
      ...
      <div style="padding:16px 18px;border-radius:16px;background:rgba(167,196,188,.07);margin-bottom:18px;">
        <p class="disp" style="margin:0;font-size:19px;line-height:1.4;font-weight:500;color:#F7F5F2;">${esc(objText)}</p>
      </div>
      ${lifeSkillHtml}
      ${actionsHtml}
      <div style="display:flex;flex-wrap:wrap;gap:7px;margin-bottom:18px;">
        ${[['S', 'Spécifique'], ['M', 'Mesurable'], ['A', 'Atteignable'], ['R', 'Réaliste'], ['T', 'Temporel']]
          .map(
            ([l, t]) =>
              `<span class="bx" style="display:inline-flex;align-items:center;gap:7px;padding:8px 12px;border-radius:12px;background:rgba(255,255,255,.04);font-size:13px;font-weight:500;color:rgba(234,243,241,.8);"><b class="disp" style="color:#F9EB50;">${l}</b> ${t}</span>`
          )
          .join('')}
      </div>
      <div style="display:flex;align-items:baseline;justify-content:space-between;margin-bottom:10px;">
        <span style="font-size:14px;font-weight:500;color:rgba(234,243,241,.7);">Progression du parcours</span>
        <span class="disp" style="font-size:16px;font-weight:600;color:#F9EB50;">${pct} %</span>
      </div>
    ...
    <div class="bx b-clk b-card b-half" data-info="focus" style="grid-column:span 2;border:1px solid rgba(249,235,80,.22);padding:24px 20px;text-align:center;${ain(9)}">${HINT}
      ...
      <p class="disp" style="margin:0;font-size:40px;line-height:1.05;font-weight:700;letter-spacing:.05em;color:#F9EB50;overflow-wrap:anywhere;animation:b-breatheC 4.6s ease-in-out infinite;">${esc(focus)}</p>
      <p style="margin:12px 0 0;font-size:13px;font-weight:600;letter-spacing:.1em;text-transform:uppercase;color:rgba(234,243,241,.6);">${focusWord ? 'Mot-ancre actif' : 'À définir en séance 9'}</p>
    </div>
    ...
    <div class="bx b-clk b-card b-half" data-info="emotions" style="grid-column:span 2;${ain(10)}">${HINT}
      <div style="display:flex;flex-wrap:wrap;align-items:baseline;justify-content:space-between;gap:4px 10px;margin-bottom:6px;padding-right:26px;">
        <span class="disp" style="font-size:19px;font-weight:600;color:#F7F5F2;">Roue des Émotions</span>
        <span style="font-size:13px;font-weight:600;color:rgba(234,243,241,.6);white-space:nowrap;flex-shrink:0;">Séances S4 · S5</span>
      </div>
      <div style="position:relative;width:150px;height:150px;margin:18px auto;">
        ...
        <div style="position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;gap:2px;">
          ${
            ent.emotionWheel
              ? `<span style="font-size:12px;font-weight:600;color:rgba(234,243,241,.6);">${latestEmotion ? 'Identifiée' : 'À explorer'}</span>
          <span class="disp" style="font-size:22px;font-weight:600;color:#F9EB50;">${esc(latestEmotion || '—')}</span>`
              : `<span style="font-size:12px;font-weight:600;color:rgba(234,243,241,.6);">Identifiée</span>
          <span class="disp" aria-hidden="true" style="font-size:22px;font-weight:600;color:#F9EB50;filter:blur(6px);user-select:none;">Trac</span>`
          }
        </div>
      </div>
      ${
        ...
        <span style="display:inline-flex;align-items:center;gap:6px;padding:6px 11px;border-radius:11px;background:rgba(249,235,80,.1);font-size:13px;font-weight:500;color:#F9EB50;"><span style="width:6px;height:6px;border-radius:50%;background:#F9EB50;"></span>Trac</span>
        ...
        <span style="display:inline-flex;align-items:center;gap:6px;padding:6px 11px;border-radius:11px;background:rgba(255,255,255,.05);font-size:13px;font-weight:500;color:rgba(234,243,241,.8);"><span style="width:6px;height:6px;border-radius:50%;background:rgba(234,243,241,.8);"></span>Détermination</span>
      </div>`
    ...
    <div class="bx b-clk b-card b-half" data-info="routine" style="grid-column:span 2;${ain(11)}">${HINT}
      <div style="display:flex;flex-wrap:wrap;align-items:baseline;justify-content:space-between;gap:4px 10px;margin-bottom:18px;padding-right:26px;">
        <span class="disp" style="font-size:19px;font-weight:600;color:#F7F5F2;">Routine pré-tir</span>
        <span style="font-size:13px;font-weight:600;color:rgba(234,243,241,.6);white-space:nowrap;flex-shrink:0;">Séance S6</span>
      </div>
      <div style="position:relative;display:flex;flex-direction:column;gap:14px;">
        ...
        ${[
          ...
          .map(
            ([n, bg, txt]) =>
              `<div style="position:relative;display:flex;align-items:center;gap:13px;"><span class="disp bx" style="z-index:1;flex-shrink:0;width:30px;height:30px;border-radius:999px;background:${bg};color:#06222A;display:grid;place-items:center;font-weight:700;font-size:13px;">${n}</span><span style="font-size:15px;font-weight:500;color:rgba(234,243,241,.86);">${txt}</span></div>`
          )
          .join('')}
        <div style="position:relative;display:flex;align-items:center;gap:13px;"><span class="disp bx" style="z-index:1;flex-shrink:0;width:30px;height:30px;border-radius:999px;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.14);color:#F7F5F2;display:grid;place-items:center;font-weight:700;font-size:13px;">4</span><span style="font-size:15px;font-weight:600;color:#F7F5F2;">Action — j'y vais</span></div>
      </div>
    </div>
    ...
    <div class="bx b-clk b-card b-half" data-info="contrat" style="grid-column:span 2;${ain(12)}">${HINT}
      <div style="display:flex;flex-wrap:wrap;align-items:baseline;justify-content:space-between;gap:4px 10px;margin-bottom:10px;padding-right:26px;">
        <span class="disp" style="font-size:19px;font-weight:600;color:#F7F5F2;">Contrat de confiance</span>
        <span style="font-size:13px;font-weight:600;color:rgba(234,243,241,.6);white-space:nowrap;flex-shrink:0;">Séance S1</span>
      </div>
      ${[
        ...
        .map(
          ([role, val], i, arr) =>
            `<div style="display:flex;align-items:center;justify-content:space-between;gap:10px;padding:12px 0;${i < arr.length - 1 ? 'border-bottom:1px solid rgba(255,255,255,.07);' : ''}"><span style="min-width:0;display:flex;flex-direction:column;gap:2px;"><span class="b-eye" style="font-size:11px;">${role}</span><span class="disp" style="font-style:italic;font-size:18px;color:#F7F5F2;overflow-wrap:anywhere;">${val}</span></span><span style="flex-shrink:0;width:26px;height:26px;border-radius:999px;background:rgba(167,196,188,.16);color:#A7C4BC;display:grid;place-items:center;line-height:0;"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="m5 12 5 5L20 7"></path></svg></span></div>`
        )
        .join('')}
      <div style="display:flex;align-items:center;gap:8px;margin-top:14px;padding-top:14px;border-top:1px solid rgba(255,255,255,.07);font-size:15px;font-weight:600;color:${docIds.contract ? '#A7C4BC' : 'rgba(234,243,241,.5)'};">${
        docIds.contract
    ...
    <div class="bx b-clk b-card" data-info="lettre" style="grid-column:span 3;${ain(13)}">${HINT}
      ...
      <div style="border-radius:16px;background:rgba(255,255,255,.035);padding:20px;">
        <p class="disp" style="margin:0 0 16px;font-style:italic;font-size:16px;color:rgba(234,243,241,.7);">Chère ${esc(firstName)},</p>
        <div style="display:flex;flex-direction:column;gap:10px;">
          <div style="height:7px;width:92%;border-radius:4px;background:rgba(234,243,241,.13);"></div>
      ...
      <p style="margin:14px 0 0;font-size:14px;font-weight:500;color:${docIds.letter ? '#A7C4BC' : 'rgba(234,243,241,.65)'};">${
        docIds.letter
    ...
    <!-- S13 · CERTIFICAT -->
    <div class="bx b-clk" data-info="certificat" style="grid-column:span 3;position:relative;border:1px dashed rgba(255,255,255,.18);border-radius:22px;padding:var(--bcard-pad,20px);${ain(14)}">${HINT}
      <div style="display:flex;flex-wrap:wrap;align-items:baseline;justify-content:space-between;gap:4px 10px;margin-bottom:14px;padding-right:26px;">
        <span class="disp" style="font-size:19px;font-weight:600;color:rgba(234,243,241,.85);">Certificat THRIVE</span>
        ${docIds.certificate && certificateReady ? okBadge('✓ disponible') : waitBadge('à venir · S13')}
      </div>
      <p style="margin:0 0 18px;font-size:15px;line-height:1.55;color:rgba(234,243,241,.68);">Reconnaissance officielle de fin de parcours — débloquée à la dernière séance.</p>
      <div style="display:flex;align-items:baseline;justify-content:space-between;margin-bottom:10px;">
        <span style="font-size:14px;font-weight:500;color:rgba(234,243,241,.7);">${completed}/13 séances</span>
        <span class="disp" style="font-size:16px;font-weight:600;color:rgba(234,243,241,.8);">${remaining} restantes</span>
      </div>
      ...
      ${
        docIds.certificate && certificateReady
          ? `<div class="b-doc b-hover" data-doc="${docIds.certificate}" style="display:flex;align-items:center;justify-content:center;gap:8px;min-height:48px;margin-top:16px;border-radius:14px;background:rgba(249,235,80,.12);color:#F9EB50;font-size:15px;font-weight:700;"><svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 4v11m0 0-4-4m4 4 4-4M5 20h14"></path></svg>Télécharger le certificat</div>`
        ...
        <span style="color:#F9EB50;flex-shrink:0;line-height:0;"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="4.5" y="10.5" width="15" height="9.5" rx="2.4"></rect><path d="M8 10.5V7.5a4 4 0 0 1 8 0v3"></path></svg></span>
        <span style="font-size:14px;line-height:1.45;color:rgba(234,243,241,.8);">Disponible en téléchargement dès la validation de la séance 13.</span>
      </div>`
      }
    </div>
  </div>
