...
// Liberté totale : les 39 fiches cœur, sans verrou de semaine ; les compléments
// dès que leur semaine est ouverte, les bonus dans leur propre rayon. Les onglets d'âge
...
import { isVisible, renderOpener, type AgeBand, type P3Activity } from '@/lib/p3-moments';
import { P3_BASE, p3Pool } from '@/lib/p3-moments/app';
import { BAND_LABELS, FILTERS, byProgramme, matchesFilter, weekSpan, type FilterId } from '@/lib/p3-moments/shelves';
...
function Catalogue({ ctx }: { ctx: P3Ctx }) {
  ...
  const all = useMemo(() => p3Pool().filter((a) => isVisible(a, data.openWeek)), [data.openWeek]);
  ...
  const list = useMemo(() => {
    ...
    return order === 'nouvelles'
      ? [...kept].sort((a, b) => Number(data.doneIds.has(a.id)) - Number(data.doneIds.has(b.id)) || byProgramme(a, b))
      : [...kept].sort(byProgramme);
  ...
  return (
    <div>
      ...
      {list.length === 0 ? (
        ...
        [...PHASES, { id: null, label: 'Bonus · hors programme' }].map((p) => {
          const items = list.filter((a) => a.phase === p.id);
          if (!items.length) return null;
          return (
            <section key={p.id ?? 'bonus'} className="mt-9">
