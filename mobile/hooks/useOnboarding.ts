...
export type OnboardingStep = { key: string; title: string; completed: boolean }
...
export function useOnboarding() {
  ...
  const stepsWithStatus = steps.map((s) => ({ ...s, completed: completedSteps.includes(s.key) }))

  const loadProgress = useCallback(async () => {
    if (!user) return
    const { data } = await supabase
      .from('onboarding_progress').select('step_key').eq('user_id', user.id)
  ...
  const completeStep = useCallback(async (stepKey: string) => {
    ...
    try {
      ...
      if (steps.every((s) => newCompleted.includes(s.key))) {
  ...
  return {
    ...
    isOnboardingDone: profile?.onboarding_completed === true,
    loading,
    loadProgress,
    completeStep,
    skipOnboarding,
